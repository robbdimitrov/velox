...
<div class="min-h-screen text-ink pb-10" data-theme="velox">
  <div class="fixed inset-0 z-[-1] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-signal/20 via-carbon to-carbon pointer-events-none"></div>
  <header class="sticky top-4 z-30 mx-auto max-w-7xl px-4 pt-2 pb-6">
    <nav class="glass-panel flex h-16 items-center justify-between px-6 shadow-glow">
      <a href="/" class="flex items-center gap-2 text-2xl font-black uppercase tracking-tight hover:scale-105 transition-transform">
        <div class="p-2 bg-gradient-to-br from-signal to-accent rounded-xl shadow-lg shadow-signal/30">
      ...
      <div class="flex items-center gap-4">
        ...
        {#if authState.role === 'reserver'}
          <a class="btn btn-sm border-white/10 bg-black/40 text-ink hover:border-signal hover:bg-signal/20 rounded-lg shadow-sm backdrop-blur-md transition-all duration-300" href="/wallet">
            <ShieldCheck size={16} class="text-ok" /> Wallet
          </a>
        {:else}
          <a class="btn btn-sm border-white/10 bg-black/40 text-ink hover:border-signal hover:bg-signal/20 rounded-lg shadow-sm backdrop-blur-md transition-all duration-300" href="/vendor">
