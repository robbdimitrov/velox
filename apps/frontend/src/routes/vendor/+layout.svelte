<script lang="ts">
  ...
  const links = [
    ...
    { href: '/vendor/events/new', icon: Calendar, label: 'Create Event' },
    { href: '/vendor/venues', icon: MapPin, label: 'Venues' },
    ...
    >
      ...
      <nav class="flex-1 space-y-2">
        {#each links as link}
          <a
            href={link.href}
            class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 {page
              .url.pathname === link.href ||
            (link.href !== '/vendor' && page.url.pathname.startsWith(link.href))
