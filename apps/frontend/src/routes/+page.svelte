...
<main
  class="mx-auto grid max-w-7xl gap-4 px-4 py-5 lg:grid-cols-[260px_1fr_340px]"
>
  <aside class="thin-panel h-max p-4">
    ...
    <div class="mt-4 space-y-4">
      ...
      <label class="form-control">
        ...
        <input
          class="input input-bordered input-sm border-line bg-carbon"
          type="date"
        />
      </label>
      <label class="form-control">
        ...
        <div
          class="flex items-center gap-2 border border-line bg-carbon px-2 py-1"
        >
          <MapPin size={15} />
          <input
      ...
      <div class="flex items-end justify-between border-b border-line pb-3">
        <div>
          ...
          <p class="text-sm text-ink/60">
            Read-model discovery, cached for flash sale pressure.
          </p>
        </div>
  ...
  <aside class="thin-panel h-max p-4">
    ...
    <div class="mt-3 grid gap-3">
      {#each data.discovery.featured as event}
        <a
          class="group block border border-line bg-carbon"
          href={`/events/${event.id}`}
        >
          <img class="h-32 w-full object-cover" src={event.image_url} alt="" />
          <div class="p-3">
            <p class="font-black uppercase group-hover:text-signal">
              {event.title}
            </p>
