    ...
    <div class="grid gap-6 lg:grid-cols-[400px_1fr]">
      <div class="space-y-4">
        {#each data.wallet.tickets as ticket}
          <article
            class="glass-panel rounded-2xl overflow-hidden shadow-lg border border-white/10 bg-gradient-to-b from-black/60 to-black/30 hover:border-signal/40 transition-colors group"
          >
            <div class="flex gap-5 p-5">
              <div
                class="grid h-24 w-24 shrink-0 place-items-center rounded-xl bg-white text-carbon shadow-inner group-hover:scale-105 transition-transform"
              >
                <QrCode size={64} />
              </div>
              <div class="min-w-0 flex-1 flex flex-col justify-center">
                ...
                <div
                  class="mt-2 inline-block bg-black/40 border border-white/5 rounded-lg px-2 py-1 font-mono text-xs font-bold text-white shadow-sm"
                >
            ...
            >
              ...
              <button
                class="btn btn-sm rounded-lg border-0 bg-white/10 hover:bg-signal text-white font-bold transition-all disabled:opacity-30 disabled:hover:bg-white/10"
      ...
      <div class="glass-panel p-6 h-max">
        ...
        <div class="space-y-4">
          {#each data.wallet.tickets as ticket}
            <details
              class="group rounded-xl border border-white/5 bg-black/30 overflow-hidden"
              open
            >
              <summary
              ...
              <div class="p-4 space-y-2">
                {#each ticket.ledger as row}
                  <div
                    class="grid gap-2 rounded-lg border border-white/5 bg-black/20 p-3 text-xs md:grid-cols-[180px_1fr_120px] items-center hover:bg-white/5 transition-colors"
                  >
