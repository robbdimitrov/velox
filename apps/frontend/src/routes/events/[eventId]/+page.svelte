<script lang="ts">
  ...
  let eventLog = $state<string[]>([
    'Snapshot loaded from projection read model'
  ]);
...
>
  <aside class="thin-panel h-max p-4">
    ...
    <div class="mt-4 grid grid-cols-2 gap-2">
      <button class="btn btn-sm border-line bg-panel"
    ...
    <div
      class="mt-4 space-y-2 border-t border-line pt-4 text-xs uppercase text-ink/60"
    >
      ...
      <p>
        <span class="inline-block h-2 w-2 bg-carbon ring-1 ring-line"></span> Sold
      </p>
    </div>
  </aside>
    ...
    <div class="mb-3 flex items-end justify-between">
      <div>
        ...
        <p class="text-sm text-ink/60">
          {data.event.venue} · section {sectionID}
        </p>
      </div>
      <p class="font-mono text-xs text-ink/50">
        snapshot {data.snapshot.snapshot_age_ms}ms · lag {data.snapshot
          .projection_lag_ms}ms
      </p>
    </div>
    <SeatCanvas
  ...
  <aside class="thin-panel h-max p-4">
    ...
    <div class="mt-4 min-h-36 space-y-2">
      {#if seatState.selectedSeats.length}
        {#each seatState.selectedSeats as seat}
          <div
            class="flex items-center justify-between border border-line bg-carbon p-2 font-mono text-sm"
          >
    ...
    <div class="mt-4 border-t border-line pt-4">
      ...
      {#if error}
        <p class="mt-3 border border-urgency px-2 py-1 text-sm text-urgency">
          {error}
        </p>
      {/if}
      <button
