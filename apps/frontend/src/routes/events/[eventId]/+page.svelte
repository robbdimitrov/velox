...
<main class="mx-auto grid max-w-7xl gap-6 px-4 py-8 lg:grid-cols-[260px_1fr_320px]">
  <aside class="glass-panel h-max p-6 sticky top-28">
    <div class="flex items-center gap-3 border-b border-white/10 pb-4 mb-6">
      ...
      <h2 class="text-sm font-black uppercase tracking-wider text-white">Section Tools</h2>
    </div>
    ...
    <div class="space-y-3 border-t border-white/10 pt-6 text-xs uppercase font-bold tracking-wider text-inkMuted">
      <p class="flex items-center gap-3"><span class="inline-block h-3 w-3 rounded-full bg-[#9CA3AF]"></span> Available</p>
      <p class="flex items-center gap-3"><span class="inline-block h-3 w-3 rounded-full bg-signal shadow-[0_0_8px_rgba(124,58,237,0.8)]"></span> Selected</p>
      <p class="flex items-center gap-3"><span class="inline-block h-3 w-3 rounded-full bg-accent shadow-[0_0_8px_rgba(255,42,95,0.8)]"></span> Held</p>
      <p class="flex items-center gap-3"><span class="inline-block h-3 w-3 rounded-full bg-[#15151A] border border-white/20"></span> Sold</p>
    </div>
  </aside>

  <section class="min-w-0 flex flex-col gap-4">
    <div class="glass-panel p-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
      <div>
        <h1 class="text-3xl font-black uppercase tracking-tight text-white drop-shadow-md">{data.event.title}</h1>
        <p class="text-sm font-medium text-signal mt-1 uppercase tracking-wide">
          {data.event.venue} <span class="text-inkMuted mx-2">|</span> Section {sectionID}
        </p>
      </div>
      <div class="bg-black/40 px-3 py-1.5 rounded-full border border-white/5 flex flex-col items-end">
        <p class="font-mono text-xs text-ink/60 uppercase tracking-widest">Snapshot <span class="text-white">{data.snapshot.snapshot_age_ms}ms</span></p>
        <p class="font-mono text-xs text-ink/60 uppercase tracking-widest mt-0.5">Lag <span class="text-ok">{data.snapshot.projection_lag_ms}ms</span></p>
      </div>
    </div>
    ...
    <div class="glass-panel h-32 overflow-hidden p-4 relative">
      ...
      <div class="space-y-1.5">
        {#each eventLog as item, index (item)}
          <p class="font-mono text-xs text-ink/80 truncate hover:text-white transition-colors" in:slide={{duration: 200}}>
  ...
  <aside class="glass-panel h-max p-6 sticky top-28">
    ...
    <div class="mt-6 border-t border-white/10 pt-6">
      ...
      {#if error}
        <p class="mt-4 rounded-lg border border-urgency/50 bg-urgency/10 p-3 text-xs font-medium text-urgency leading-tight">
          {error}
        </p>
      {/if}
