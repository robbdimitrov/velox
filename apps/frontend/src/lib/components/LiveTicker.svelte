...
<section class="thin-panel grid h-28 grid-cols-[44px_1fr] overflow-hidden">
  <div class="flex items-center justify-center border-r border-line text-urgency">
    <Activity size={20} />
  </div>
  <div class="divide-y divide-line">
    {#each messages as message}
      <p class="h-7 truncate px-3 py-1 font-mono text-xs uppercase text-ink/70">{message}</p>
    {/each}
  </div>
</section>
